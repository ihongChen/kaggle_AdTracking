...
train_cols = ['ip', 'app', 'device', 'os',
              'channel', 'is_attributed', 'click_time']
train_df = pd.read_csv(path + 'train.csv.zip', skiprows=range(1,
                                                              84903891), nrows=100000000, dtype=dtypes, usecols=train_cols)
...
test_df = pd.read_csv(path + "test.csv.zip", dtype=dtypes, usecols=test_cols)
...
print('write importance...')
feat_imp = pd.DataFrame({'f': model.feature_name(),
                         'split': model.feature_importance(importance_type='split'),
                         'gain': model.feature_importance(importance_type='gain')}).sort_values(by='split', ascending=False)
feat_imp.to_csv('../find/feat_impact9791_1.csv', index=False)
print('finished!!')
# print('Predicting...')
